...
def test_default_path(cache_folder_creation):
    ...
    path_resolver = OsPathResolver().resolve_path()
    default_folder = cache_folder_creation().path_to_folder
...
def test_valid_custom_path(tmp_path, cache_folder_creation):
    custom_folder = cache_folder_creation(tmp_path).path_to_folder
...
def test_not_valid_custom_path(cache_folder_creation):
    with pytest.raises(OsPathResolverError):
        getattr(cache_folder_creation("not/valid/path"), "path_to_folder")
